...
class Application extends App
{
    public function __construct(array $urlParams = array())
    {
        ...
        $container->registerAlias('OCA\OCA\Dashboard\Controller\AdminController', AdminController::class);

        $container->registerService('OCA\Activity\ViewInfoCache', function(IAppContainer $c) {
            return new ViewInfoCache(
                new View('')
            );
        });
        $container->registerService('OCA\Activity\isCLI', function() {
            return \OC::$CLI;
        });
        $container->registerService('OCP\IUserManager', function(IAppContainer $c) {
            return $c->getServer()->getUserManager();
        });
        $container->registerService('Symfony\Component\EventDispatcher\EventDispatcherInterface', function(IAppContainer $c) {
            return $c->getServer()->getEventDispatcher();
        });
        $container->registerService('OCP\Security\ISecureRandom', function(IAppContainer $c) {
            return $c->getServer()->getSecureRandom();
        });
        $container->registerService('CurrentUID', function (IAppContainer $c) {
            $user = $c->getServer()->getUserSession()->getUser();
            return ($user) ? $user->getUID() : '';
        });
        $container->registerService('OCP\IDBConnection', function (IAppContainer $c) {
            return $c->getServer()->getDatabaseConnection();
        });
        $container->registerService('OCA\DAV\Connector\Sabre\Principal', function (IAppContainer $c) {
